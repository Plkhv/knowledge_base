...
def create_spark_session() -> SparkSession:
    ...
    jars = ",".join([
        ...
        f"{jar_dir}/iceberg-spark-runtime-3.5_2.12-1.4.0.jar",
        f"{jar_dir}/iceberg-nessie-1.4.0.jar",
        f"{jar_dir}/nessie-client-0.7.0.jar",
    ])

    spark = (
        SparkSession.builder
        .appName("mine_metrics_calculator")
        .master("local[*]")  # ← ИСПРАВЛЕНО: используем local режим
        ...
        # ИСПРАВЛЕНО: используем HadoopFileIO вместо S3FileIO
