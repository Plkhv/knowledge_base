...
Spark-задача вычисления расчётных показателей через Trino JDBC
"""
...
def create_spark_session() -> SparkSession:
    """Создаёт SparkSession с доступом к Trino JDBC"""
...
def read_table(spark: SparkSession, table: str):
    """Читает таблицу через Trino JDBC"""
...
def write_table(df, table: str, mode: str = "overwrite"):
    """Пишет таблицу через Trino JDBC"""
    jdbc_url = f"jdbc:trino://{TRINO_HOST}:{TRINO_PORT}/{TRINO_CATALOG}/{TRINO_SCHEMA}"
    
    if mode == "overwrite":
        df.write.mode("overwrite").jdbc(jdbc_url, table, properties={
            "user": TRINO_USER,
            "driver": "io.trino.jdbc.TrinoDriver"
        })
    else:
        df.write.mode("append").jdbc(jdbc_url, table, properties={
            "user": TRINO_USER,
            "driver": "io.trino.jdbc.TrinoDriver"
        })
